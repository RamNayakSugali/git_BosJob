...
class SearchedCource extends StatefulWidget {
  int id;
  SearchedCource({super.key,required this.id});

  @override
  State<SearchedCource> createState() => _SearchedCourceState();
}

class _SearchedCourceState extends State<SearchedCource> {
  @override
  void initState() {
    profileHandler();
    getCourceData(widget.id);
    super.initState();
  }
  bool isCourceget = false;

  getCourceData(int id)async{
setState(() {
      isCourceget = true;
    filterCategory.clear();
    });
    Map data = await Services.getCourceData(id);
    ...
    if (data["message"] != null) {
      ...
      browseCourse = data["data"]["data"];
      originalBrowseCourse = data["data"]["data"];
    }
    setState(() {
      isCourceget = false;
    });
